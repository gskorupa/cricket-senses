 ...
 *
 * <p>
 * This GPIO provider implements the MCP3008 SPI GPIO expansion board as native
 * Pi4J GPIO pins. It is a 10-bit ADC providing 8 input channels. More
 ...
 * 8 GPIO pins that can be used for analog input pins. The values returned are
 * in the range 0-1023 (max 10 bit value).
 * </p>
 *
 * @author pojd
 */
public class MCP3008GpioProvider extends GpioProviderBase implements GpioProvider {
    ...
    public MCP3008GpioProvider(int spiChannel) throws IOException {
        if (spiChannel == 0) {
            this.spiDevice = SpiFactory.getInstance(SpiChannel.CS0);
        }else{
            this.spiDevice = SpiFactory.getInstance(SpiChannel.CS1);
        }
    }
